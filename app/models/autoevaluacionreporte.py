...
class AutoevaluacionReporte(db.Model):
    ...
    id_aereporte: Mapped[int] = mapped_column(
        Integer,
        Sequence('seq_autoevaluacion_reporte'),
        primary_key=True
    )
    # Foránea a evaluación
    ...
    id_condicion: Mapped[int] = mapped_column(
        ForeignKey(
            "condicion_criterio.id_condicion",
            ondelete="CASCADE"
        ),
        nullable=False
    )
    ...
    # Foránea a tipo reporte
    id_tiporeporte: Mapped[int] = mapped_column(
        ForeignKey(
            "tipo_reporte.id_tiporeporte",
            ondelete="CASCADE"
        ),
        nullable=False
    )
    ...
    link_reporte: Mapped[str] = mapped_column(
        String(1000),
        nullable=True
    )
    puntaje_reporte: Mapped[int] = mapped_column(
        Integer,
        nullable=True
    )
